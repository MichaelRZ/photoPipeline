...
// Serialization support for cv_custom::Matx
template<typename _Tp, int m, int n>
inline bool Save(ArchiveSave& a, const cv_custom::Matx<_Tp,m,n>& _m) {
	a.stream.write((const char*)_m.val, sizeof(_Tp)*m*n);
	return true;
}
template<typename _Tp, int m, int n>
inline bool Load(ArchiveLoad& a, cv_custom::Matx<_Tp,m,n>& _m) {
	a.stream.read((char*)_m.val, sizeof(_Tp)*m*n);
	return true;
}

// Serialization support for cv_custom::Point3_
template<typename _Tp>
inline bool Save(ArchiveSave& a, const cv_custom::Point3_<_Tp>& pt) {
	a.stream.write((const char*)&pt.x, sizeof(_Tp)*3);
	return true;
}
template<typename _Tp>
inline bool Load(ArchiveLoad& a, cv_custom::Point3_<_Tp>& pt) {
...
struct Interface
{
	typedef cv_custom::Point3_<float> Pos3f;
	typedef cv_custom::Point3_<double> Pos3d;
	typedef cv_custom::Matx<double,3,3> Mat33d;
	typedef cv_custom::Matx<double,4,4> Mat44d;
	typedef cv_custom::Point3_<uint8_t> Col3; // x=B, y=G, z=R
	/*----------------------------------------------------------------*/
